...
当创建一个 Router 实例，你可以提供一个 `scrollBehavior` 方法，`scrollBehavior` 函数接收 `to` 和  `from` 路由对象，如 Navigation Guards。第三个参数 `savedPosition`，只有当这是一个 `popstate` 导航时才可用（由浏览器的后退/前进按钮触发）。
