...
- Broker：服务代理节点。负责将收到的消息存储到磁盘中；
  ...
  - Partition：分区是主题在**存储层面的实现**，可以看作一个**可追加的日志（Log）文件**
    - 主题可以细分为多个分区，一个分区只属于单个主题，很多时候也会把分区称为**主题分区（Topic-Partition）**。
    - 分区实现了对**主题的水平拓展**，解决了主题单文件I/O的性能瓶颈
    ...
    - Replica：Kafka 为分区引入了多副本（Replica）机制，通过增加副本数量可以提升**容灾能力**。同一分区的不同副本中保存的是相同的消息（在同一时刻，副本之间并非完全一样），副本之间是“一主多从”的关系
      - 副本角色
        - **leader 副本**
          - **负责处理读写请求**
