...
自动位移提交的动作是在 `poll()` 方法的逻辑里完成的，在每次真正向服务端发起拉取请求之前会检查可以进行位移提交（提交时间间隔是否超过 `auto.commit.interval.ms`），如果可以则将拉取到的每个分区中下一个要拉取的消息位移（即 `position()` 方法的返回值）进行提交。
...
第①行的 `endOffsets()` 方法用来获取指定分区的末尾的消息位置(即 LEO，将要写入最新消息的位置)。
